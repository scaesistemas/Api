...
{
    public class NumeroExtensoHelper
    {
        // O método EscreverExtenso recebe um valor do tipo decimal
        public static string EscreverExtenso(decimal valor, bool valorFinanceiro = true)
        {
            if (valor == 0)
                return "ZERO";
...
| valor_por_extenso.Substring(valor_por_extenso.Length - 8, 7) == "TRILHÕES")
        ...
        public static string SubstituirNumerosPorExtenso(string texto)
        {
            ...
            string output = regex.Replace(texto, m => Escrever_Valor_Extenso(Convert.ToDecimal(m.Value)));
